...
$from_airport = $from_to_data['from'];   //assigning the from value
$destination_airport = $from_to_data['to'];   //assigning the to value
    ...
    <form action="trip-search.php" method = 'POST'>
      <div class = 'choose1'>
        <select id="going" class="trip" value = 'going' name = 'going' >
          <option disabled selected value="">Please select a Going Trip</option>
          <?php if(empty ($trips)){ ?>
      ...
      <div class = 'choose2'>
        <select id="return" class="rtrip" value = 'return' name = 'return'>
          <option value="">Please select a returning Trip</option>
          <?php if(empty($Rtrips)){ ?>
          <div class = 'red-error'>Sorry there are no trips matching your search</div>
          <?php }?>
